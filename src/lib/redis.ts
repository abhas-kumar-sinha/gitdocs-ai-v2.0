...
export const publishProgress = async (projectId: string, data: ProgressEvent) => {
  await redis.publish(`project:${projectId}:progress`, JSON.stringify(data));
};

export const subscribeToProgress = (projectId: string, callback: (data: ProgressEvent) => void) => {
