  ...
  const updateProject = useMutation(
    trpc.project.updateStarred.mutationOptions({
      ...
      onSuccess: ({ id, isStarred }) => {
        setProjects((prev) =>
          prev.map((project) =>
            project.id === id ? { ...project, isStarred } : project,
          ),
        );
        ...
        return (
          ...
          >
            <Button
              ...
              size="icon-sm"
              className={cn(
                "absolute border-none top-2 right-2 z-10 transition-all duration-200",
                project.isStarred ? "" : "opacity-0 group-hover:opacity-100",
              )}
            >
              {project.isStarred ? (
                <Star fill="#EAB308" stroke="none" />
              ) : (
                <Star />
              )}
            </Button>
            ...
            <div
              ...
              className="relative aspect-video w-full overflow-hidden rounded-xl bg-muted/10 border border-border/40 shadow-sm"
            >
              <MarkdownPreview
