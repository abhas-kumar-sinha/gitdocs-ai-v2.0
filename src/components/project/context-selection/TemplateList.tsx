...
const TemplateList = ({
  ...
  selected: TemplateId;
  setSelected: (template: TemplateId) => void;
}) => {
  return (
      ...
      <div className="flex-1 overflow-y-auto px-2 pt-4 pb-2 space-y-3 -ms-0.5 -me-1">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-1">
                <span
                  className={`font-semibold ${selected === "ai-gen" ? "text-primary" : "text-foreground"}`}
                >
                  Let AI Generate
                </span>
                {selected === "ai-gen" && (
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                We&apos;ll analyze your codebase to create a tailored README
                automatically.
              </p>
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="px-2 text-muted-foreground">Manual Selection</span>
          </div>
        </div>
            ...
            return (
              ...
              >
                <div className="flex gap-4">
                  {/* Visual Preview */}
                  <div
                    className={`w-16 h-20 shrink-0 shadow-sm transition-opacity duration-200 ${isSelected ? "opacity-100" : "opacity-70 group-hover:opacity-100"}`}
                  >
                  ...
                  <div className="flex-1 min-w-0 flex flex-col">
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {template.tags.slice(0, 3).map((tag) => (
                        <span
                          key={tag}
                          className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-muted text-muted-foreground border border-border/50"
                        >
                          {tag}
                        </span>
                      ))}
