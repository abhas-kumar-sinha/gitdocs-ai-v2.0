...
const ProjectPage = () => {
  ...
  const filteredProjects = useMemo(() => {
    ...
    // Apply sorting
    data.sort((a, b) => {
      let valA = a[deferredSortBy];
      let valB = b[deferredSortBy];

      if (deferredSortBy === "updatedAt" || deferredSortBy === "createdAt") {
      ...
      if (valA < valB) return deferredOrderBy === "asc" ? -1 : 1;
      if (valA > valB) return deferredOrderBy === "asc" ? 1 : -1;
      return 0;
    });

    return data;
  }, [initialProjects, deferredSearchQuery, deferredSortBy, deferredOrderBy]);
